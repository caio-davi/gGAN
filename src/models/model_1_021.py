...
def define_discriminator(in_shape=(96,1), n_classes=2):
    # image input
    in_sample = Input(shape=in_shape)
    # fully connected layers
    fe = Dense(180, activation='relu')(in_sample)
    fe = Dense(360, activation='relu')(fe)
    fe = Dense(180, activation='relu')(fe)
...
def define_generator(latent_dim):
    ...
    gen = Dense(90, activation='relu')(gen)
    gen = UpSampling1D(size=2)(gen)
    gen = Dense(180, activation='relu')(gen)
    gen = UpSampling1D(size=2)(gen)
    gen = Dense(96, activation='sigmoid')(gen)

    out_layer = Reshape((96, 1))(gen)
...
# plot_model(g_model, to_file='./images/generator_021.png', show_shapes=True, show_layer_names=True)